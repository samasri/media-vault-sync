...
func TestEndToEnd_HappyPath_VideoIngest(t *testing.T) {
	...
	queue.Subscribe(ctx, "onprem:p1:albummanifestupload", "albummanifestupload", "p1", albumManifestUploadConsumer.Handle)
	...
	album, err := albumRepo.FindByAlbumUID(ctx, "p1", "db1", "album1")
	if err != nil {
	...
	if vid2 == nil {
		t.Error("video metadata for v2 should exist")
	}
}

type mediaVaultRegistryProxyForReceiver struct {
	getRegistry func() services.MediaVaultRegistry
}

func (r *mediaVaultRegistryProxyForReceiver) Get(databaseID string) (services.MediaVault, error) {
