...
func TestUnexpectedVideo_MarksUnsynced(t *testing.T) {
	...
	var albumManifestUploadProcessed int32
	wrappedAlbumManifestUploadConsumer := func(ctx context.Context, msg services.Message) error {
		...
		atomic.AddInt32(&albumManifestUploadProcessed, 1)
	...
	if album.Synced {
		t.Error("album should be unsynced because v2 was not in manifest")
	}
	...
	if obj1 == nil {
		t.Error("object for v1 should exist (it was in manifest)")
	}
	if obj2 != nil {
		t.Error("object for v2 should NOT exist (it was rejected)")
	}
}
